...
async def run_llm_service():
    """Poll for news_sampled keywords and run combined justification+enrichment in one LLM call."""
    logger.info(f"LLM service started. Poll interval={LLM_POLL_INTERVAL_SECONDS}s, batch size={LLM_BATCH_SIZE}")
    client = OpenRouterClient()

    while True:
        try:
            logger.info(f"[LLM] Polling batch | status=news_sampled | batch_size={LLM_BATCH_SIZE}")
            async with get_session() as session:
                async with session.begin():
                    ...
                    for kw in keywords:
                        await process_keyword(kw, list(kw.articles), client, session)
                    logger.info(f"[LLM] Batch complete | processed={len(keywords)}")

            with open("/tmp/llm_heartbeat.txt", "w") as f:
                f.write(str(time.time()))

        except Exception as e:
            logger.error(f"LLM service loop error: {e}")
